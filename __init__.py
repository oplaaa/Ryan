...
yobot_url = 'http://175.24.70.191:9222' #请修改为你的yobot网址 例：http://xxx.xx.xxx.xxx:xxxx
year = datetime.now().strftime('%Y')#年
month = int(datetime.now().strftime('%m'))#去0的月
sv = Service('离职报告')
date = datetime.today()
def getTheMonth(month):
    if month == 1:
        pcr_month = 12
    else:
        pcr_month = month-1
    return pcr_month
pcr_month = str(getTheMonth(month))
month = str(month)
def pcr_constellations(month):
    ...
    j_constellations = ["水瓶","双鱼","白羊","金牛","双子","巨蟹","狮子","处女","天秤","天蝎","射手","摩羯"]#日服的
    if game_server =="cn":
        for x in range(1,13):
            if x == int(month)-1:
...
async def create_resignation_report(bot, event):
    ...
    add_text(img, pcr_month, position=(565,438), textsize=40)
